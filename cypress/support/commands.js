...
Cypress.Commands.add('logInApi', () => {
  ...
  cy.fixture(fixtureName).then((user) => {
    cy.request({
      ...
      body:
      {
        "email": user.validCredentials.email,
...
Cypress.Commands.add('addExpenses', (expenseData) => {
  fuelExpensesP.addExpensesButton.click();
  fuelExpensesF.mileageInput.clear().type(expenseData.mileage);
  fuelExpensesF.numberOfLitersInput.type(expenseData.liters);
  fuelExpensesF.totalCostInput.type(expenseData.totalCost);
  fuelExpensesF.addButton.click();
});

Cypress.Commands.add('addExpenses v2', (newMileage, liters, cost) => {
  fuelExpensesP.addExpensesButton.click();
  fuelExpensesF.mileageInput.type(newMileage);
  fuelExpensesF.numberOfLitersInput.type(liters);
  fuelExpensesF.totalCostInput.type(cost);
  fuelExpensesF.addButton.click();
});
